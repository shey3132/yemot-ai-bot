...
def get_model():
    # פה השינוי הקריטי: מעבירים את הכלי כמילון (dict) ולא כמחרוזת
...
def ai_chat():
    ...
    try:
        # יצירת המודל בכל פעם כדי למנוע בעיות טעינה
        model = get_model()
        audio_file = genai.upload_file(path=tmp_filename)
        
        # יצירת תשובה מה-AI
        response = model.generate_content(["אתה עוזר קולי חכם שמחובר לאינטרנט.", audio_file])
        ...
        print(f"Error: {e}") # זה יודפס בלוגים של Render
