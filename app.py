...
def ai_chat():
    ...
    if not audio_path:
        welcome_msg = "שלום וברכה הגעתם לנועם העוזר החכם שלכם אני משוחח איתכם אנא דברו לאחר הצליל"
        return Response(
    ...
    try:
        ...
        # הרחבת התיקון האוטומטי לכל השיבושים האפשריים של ישעיהו מהלוגים
        bad_names = ["אישייהו", "שיור", "אישי אהו", "ישעיה", "ישיהו", "משיהו", "מישהו", "משהו"]
        if user_text in bad_names or known_name in bad_names:
            known_name = "ישעיהו"
        ...
        # =========================
        # SYSTEM PROMPT & AI CHAT (משופר וגמיש לבקשות)
        # =========================
        system_prompt = (
            "קוראים לך נועם. אתה עוזר קולי חכם, קצר, אנושי וזורם בטלפון. "
            "נוצרת על ידי שי מניהול פרויקטים כדי לסייע לאחרים בניהול משימות ופניות. "
            "חוקים חשובים לניהול השיחה:\n"
            "1. זרום תמיד עם בקשת המשתמש מיד! אם המשתמש מבקש ממך לספר סיפור, בדיחה או לדבר על נושא מסוים, אל תתווכח ואל תגיד שאתה לא מבין. פשוט תעשה את זה על המקום ובקצרה.\n"
            "2. ענה תמיד בקצרה (משפט אחד או שניים בלבד). אל תנאם נאומים, אל תציע עזרה בצורה רובוטית וחזרתית שוב ושוב.\n"
            ...
            "5. תן תשובות מלאות וברורות ללא קיצור יתר אך בלי לחפור.\n"
        )
            # הזרקת השם לתוך ה-System Prompt אם הוא קיים ב-DB
if known_name:
    system_prompt += f" שם המשתמש הוא {known_name}. פנה אליו בשמו."
else:
    system_prompt += " אם אינך יודע את השם, שאל פעם אחת בלבד עם מי אני מדבר."
